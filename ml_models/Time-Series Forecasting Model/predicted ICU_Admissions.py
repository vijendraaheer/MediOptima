...
# ==============================
# LOAD DATA
df = pd.read_csv("data/processed/clean_hospital_data.csv")  # Must have "Date", "ICU Admissions", "Patients", "Discharge Count"
...
for lag in [1,3,7,14,21,30]:
    df[f"lag{lag}"] = df["ICU Admissions"].shift(lag)

# Rolling features
for window in [3,7,14,30]:
    df[f"rolling{window}"] = df["ICU Admissions"].rolling(window).mean()
...
data = df[[
    "Date","ICU Admissions","Patients","Discharge Count",
...
# LSTM MODEL
scaler = MinMaxScaler()
scaled = scaler.fit_transform(df[["ICU Admissions"]])
sequence = 14
...
print("\nHYBRID MODEL PERFORMANCE FOR ICU ADMISSIONS")
print("MAE:", round(mae,2))
print("RMSE:", round(rmse,2))
print("MAPE:", round(mape,2), "%")

# ==============================
# FUTURE FORECAST JAN 2025
patient_data = pd.read_csv("ml_models\Time-Series Forecasting Model/prediced_patients.csv")
discharge_data = pd.read_csv("ml_models\Time-Series Forecasting Model/Expected_discharge.csv") 
...
df_forecast.columns = ["Date","Predicted_ICU_Admissions"]
df_forecast["Predicted_ICU_Admissions"] = df_forecast["Predicted_ICU_Admissions"].clip(lower=0)

df_forecast.to_csv("ml_models\Time-Series Forecasting Model/Predicted_ICU_Admissions.csv", index=False)
print("\nJanuary 2025 ICU Admissions Forecast")
print(df_forecast.head())

# ==============================
# PLOT
plt.figure(figsize=(10,5))
plt.plot(actual, label="Actual ICU Admissions")
plt.plot(prophet_pred, label="Prophet")
plt.plot(lstm_pred, label="LSTM")
plt.plot(hybrid_pred, label="Hybrid", linewidth=3)
plt.legend()
plt.title("Hybrid ICU Admissions Forecast")
plt.show()
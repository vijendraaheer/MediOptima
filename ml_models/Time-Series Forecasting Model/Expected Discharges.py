...
df["Patients_scaled"] = scaler_outlier.fit_transform(df[["Discharge Count"]])
...
# Lag features
df["lag1"] = df["Discharge Count"].shift(1)
df["lag3"] = df["Discharge Count"].shift(3)
df["lag7"] = df["Discharge Count"].shift(7)
df["lag14"] = df["Discharge Count"].shift(14)
df["lag21"] = df["Discharge Count"].shift(21)
df["lag30"] = df["Discharge Count"].shift(30)


# Rolling features
df["rolling3"] = df["Discharge Count"].rolling(3).mean()
df["rolling7"] = df["Discharge Count"].rolling(7).mean()
df["rolling14"] = df["Discharge Count"].rolling(14).mean()
df["rolling30"] = df["Discharge Count"].rolling(30).mean()
...
data.columns = [
"ds","y",
"day_of_week","month","week_of_year","Patients",
"lag1","lag3","lag7","lag14",'lag21','lag30',
"rolling7","rolling14","rolling3","rolling30",
"weekend"
]
...
# LSTM MODEL
scaler = MinMaxScaler()
scaled = scaler.fit_transform(df[["Discharge Count"]])
sequence = 14
...
plt.plot(actual, label="Actual Discharge Count")
...
# Create future dataframe directly
future_df = patient_data.rename(columns={"Date": "ds"})
...
df1.columns = ["Date","Expected_discharge"]

df1.to_csv("ml_models\Time-Series Forecasting Model/Expected_discharge.csv", index=False)

print("\nJanuary 2025 discharge Forecast")
print(df1.head())